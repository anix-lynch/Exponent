# ML Engineer Interview Prep

## 📚 Resources

**250 ML Engineer questions** from [Exponent](https://www.tryexponent.com/questions?page=1&role=ml-engineer)

## 📁 Output Files

### 1. [ML_Engineer_Question_Bank.md](./ML_Engineer_Question_Bank.md)
- **250 questions** organized by category
- **ASCII framework** for each category
- **Rich text** formatting

### 2. [ml_engineer_framework.md](./ml_engineer_framework.md)
- **Comprehensive frameworks** for all 18 categories
- **Study strategy** guide
- **700+ lines** of detailed mental models

## 📊 Categories

```
Machine Learning........................  103 questions
Data Structures & Algorithms............   39 questions
System Design...........................   20 questions
Behavioral..............................   20 questions
Computer Vision.........................   11 questions
ML System Design........................   11 questions
Model Evaluation........................   10 questions
+ 11 more categories
```

## 🎯 How to Use

1. Read [ml_engineer_framework.md](./ml_engineer_framework.md) for frameworks
2. Practice with [ML_Engineer_Question_Bank.md](./ML_Engineer_Question_Bank.md)
3. Focus on: ML fundamentals, DSA, ML system design

---

**Source**: https://www.tryexponent.com/questions?page=1&role=ml-engineer
