# BizOps & Strategy Interview Prep

## 📚 Resources

**99 BizOps & Strategy questions** from [Exponent](https://www.tryexponent.com/questions?page=1&role=bizops)

## 📁 Output Files

### 1. [BizOps_Strategy_Question_Bank.md](./BizOps_Strategy_Question_Bank.md)
- **99 questions** organized by category
- **ASCII framework** for each category
- **Rich text** formatting

### 2. [bizops_strategy_framework.md](./bizops_strategy_framework.md)
- **Comprehensive frameworks** for all 13 categories
- **Study strategy** guide
- **600+ lines** of detailed mental models

## 📊 Categories

```
Strategy................................   36 questions
Behavioral..............................   18 questions
Problem Solving.........................   12 questions
Process Improvement.....................   10 questions
Business Analysis.......................    9 questions
Market Analysis.........................    8 questions
Analytical..............................    8 questions
+ 6 more categories
```

## 🎯 How to Use

1. Read [bizops_strategy_framework.md](./bizops_strategy_framework.md) for frameworks
2. Practice with [BizOps_Strategy_Question_Bank.md](./BizOps_Strategy_Question_Bank.md)
3. Focus on: Strategy, Business Analysis, Problem Solving

---

**Source**: https://www.tryexponent.com/questions?page=1&role=bizops
