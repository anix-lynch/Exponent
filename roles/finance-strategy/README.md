# Finance & Strategy Interview Prep

## 📚 Resources

**7 Finance & Strategy questions** from [Exponent](https://www.tryexponent.com/questions?role=finance-strategy)

⚠️ **Note**: Limited dataset (only 7 questions available on Exponent)

## 📁 Output Files

### 1. [Finance_and_Strategy_Question_Bank.md](./Finance_and_Strategy_Question_Bank.md)
- **7 questions** organized by category
- **ASCII framework** for each category
- **Rich text** formatting

### 2. [INTERVIEW_FRAMEWORK.md](./INTERVIEW_FRAMEWORK.md)
- **Frameworks** for all 6 categories
- **Study strategy** guide
- **Compact format** (due to small dataset)

## 📊 Categories

```
Behavioral..............................   3 questions
Financial Analysis......................   3 questions
Strategic Planning......................   2 questions
Financial Modeling......................   1 question
Product Strategy........................   1 question
Cost Management.........................   1 question
```

## 🎯 How to Use

1. Read [INTERVIEW_FRAMEWORK.md](./INTERVIEW_FRAMEWORK.md) for frameworks
2. Practice with [Finance_and_Strategy_Question_Bank.md](./Finance_and_Strategy_Question_Bank.md)
3. Focus on: Behavioral (STAR method), Financial Analysis, Strategic Planning

---

**Source**: https://www.tryexponent.com/questions?role=finance-strategy
