...
| P6 | 70 | `Impact × Confidence × Ease → RICE Score → Decide + Communicate` |
| P7 | 92 | `Define Options → Winners/Losers → Guardrails → Decide + Communicate` |
| P8 | 35 | `Hypothesis → Metric → Design → Run → Validate → Decide` |
| P9 | 8 | `Clarify Assumptions → Identify Risks → Validation Plan → Decide` |
| P10 | 46 | `Context → Insight → Recommendation → Next Steps` |
...
| P13B_General | 287 | `Users → Jobs → Core Loop → Key Features → Tradeoffs → Success Metrics` |
| P13B_Messaging_Communication | 16 | `Users → Jobs → Core Loop → Key Features → Tradeoffs → Success Metrics` |
| P13B_Social_Community | 49 | `Users → Jobs → Core Loop → Key Features → Tradeoffs → Success Metrics` |
| P13B_Travel_Transportation | 23 | `Users → Jobs → Core Loop → Key Features → Tradeoffs → Success Metrics` |
...
| Pattern | Count | Solving Formula |
|---------|-------|----------------|
| L1 | 26 | `Source → Freshness → Completeness → Bias → Sanity Checks` |
| L2 | 38 | `Current Load → 10× Projection → Bottlenecks → Mitigation` |
| L3 | 86 | `Cost Drivers → Benefits → Breakeven → Decide` |
| L4 | 4 | `Legal → Technical → Organizational → Timeline → Prioritize` |
| L5 | 20 | `Key Metrics → Alerts → Dashboards → Escalation` |
| L6 | 1 | `Constraints → Tradeoffs → Decision → Monitor` |
| L7 | 11 | `Entities → Relationships → Metrics → Grain → Validate` |
| L8 | 20 | `Competitors → Differentiation → Market Conditions → Strategy` |
| L9 | 10 | `Levers (Price, Volume, Churn) → Impact → Prioritize` |
| L10 | 13 | `Map Workflow → Identify Bottlenecks → Optimize → Measure` |
| L11 | 26 | `Enumerate Risks → Blast Radius → Mitigations → Monitor` |
| L12 | 5 | `Metric Moved → Proxy Validity → Gaming Risk → Decide` |
| L13 | 15 | `Conceptual Join → Aggregation Logic → Filter Logic → Output` |
| L14_DE | 29 | `Components → Data Flow → Boundaries → Scale Considerations` |
| L14_General | 128 | `Components → Data Flow → Boundaries → Scale Considerations` |
...
|---------|-------|----------------|
| 26_IGNORE | 441 | `N/A` |
| 26_BUCKET_A_HARD_IGNORE | 298 | `N/A` |
| 26_TO_PUT_BACK | 79 | `N/A` |
| 26_BUCKET_C_ML_CONCEPTS | 50 | `N/A` |
| 26_BUCKET_D_FROZEN_ZONE | 42 | `N/A` |
| 26_BUCKET_E_BEHAVIORAL | 29 | `N/A` |
| 26_BUCKET_B_VOCAB_ONLY | 24 | `N/A` |
