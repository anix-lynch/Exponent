# Executive Study Plan: P5 - Segmentation
**Approach:** GM-style, concept-level, not per-question  
**Time:** 2-3 hours total across 3 passes  
**Source:** 14 questions → 4 concept buckets → 3-4 high-impact buckets

**❤️ = "Hedgehog Answer"** - Your fallback narrative if you know nothing. Master these first!

---

## 🔍 HOW TO IDENTIFY P5 (SEGMENTATION) QUESTIONS

**Even when "segmentation" isn't mentioned, look for these keywords/phrases:**

### Explicit Segmentation Keywords:
- "segment", "segmentation", "persona", "user groups", "customer segments"
- "which users", "who should we focus on", "target audience"

### Implicit P5 Indicators:
- **User focus questions:** "which users should we focus on?", "who is most valuable?", "different users behave differently"
- **Persona questions:** "create buyer personas", "understand customers", "core audiences"
- **Market questions:** "which market segment?", "which geography?", "target segment"
- **Behavior questions:** "segment by behavior", "engagement by segment", "churn by segment"
- **Design for segments:** "design for X segment", "tailor for Y users", "customize for Z"

### P5 vs P1 Distinction:
- **P5 (Segmentation):** "Which users should we focus on?" → Focus: DEFINE segments (Persona × Behavior × Value)
- **P1 (Metric Drop):** "Orders down 25%" → Focus: DIAGNOSE why metric dropped (segment to find hot spot)

### P5 vs P2 Distinction:
- **P5 (Segmentation):** "Which users matter most?" → Focus: DEFINE and RANK segments
- **P2 (NSM + KPI):** "What metrics for this product?" → Focus: DEFINE what to measure

### P5 vs P4 Distinction:
- **P5 (Segmentation):** "Segment users for churn analysis" → Focus: DEFINE segments to analyze
- **P4 (Cohort):** "Retention dropped 35%" → Focus: COMPARE cohorts over time
...
### BUCKET 1: Classic User/Customer Segmentation
**Questions:** ~5 | **Priority:** 🟢 GREEN (Master this)

**Board Slide Bullets:**
- **What:** "Which users should we focus on?" or "How to segment customers?" - core segmentation framework
- **Framework:** (Persona × Behavior × Value) → Rank → Focus Top Segments
- **Persona (WHO):** Demographics (age, role, company size), Context (job-to-be-done, environment), Needs/constraints
- **Behavior (HOW):** Frequency (daily/weekly/occasional), Depth (light vs power usage), Lifecycle stage (new/active/mature)
- **Value (WHY):** Revenue (ARPU, LTV), Strategic value (growth, network effects), Cost/risk (support, churn risk)
- **Rank & Focus:** Impact on core metric, Size/growth potential, Effort to serve → Pick primary segment

**Concrete Examples:**
- "Segment e-commerce users: Persona (students, professionals, families) × Behavior (browsers, buyers, power shoppers) × Value (low/medium/high LTV) → Rank → Focus on power shoppers"
- "Create buyer personas: Define demographics, context, needs → Map behaviors → Assess value → Rank segments"
- "Segment for growth: Identify high-value segments (high LTV, low churn) → Focus product/features on them"

**Representative Questions (Do 5 only):**
- Q153: Can you provide details on your strategies or methods for understanding customers or core audiences?
- Q1069: How do you create buyer personas?
- Q1559: How would you segment users for a new e-commerce platform?
- Q2147: Tell me how you'd segment customers to support a growth initiative.
...
### BUCKET 2: Market/Geographic Segmentation
**Questions:** ~3 | **Priority:** 🟢 GREEN (High-yield)

**Board Slide Bullets:**
- **What:** "Which market segment?" or "Which geography?" - same framework, applied to markets
- **Persona (WHO):** Market demographics, geographic characteristics, cultural context
- **Behavior (HOW):** Market adoption patterns, usage frequency, growth trajectory
- **Value (WHY):** Market size, growth potential, revenue opportunity, strategic importance
- **Rank & Focus:** Market size × Growth rate × Value → Pick target market

**Concrete Examples:**
- "Next market segment: Persona (demographics, geography, culture) × Behavior (adoption patterns, usage) × Value (market size, growth, revenue) → Rank → Focus on top market"
...
### BUCKET 3: Engagement/Behavior Segmentation
**Questions:** ~3 | **Priority:** 🟡 YELLOW (High-yield but needs practice)

**Board Slide Bullets:**
- **What:** "Which content/behavior drives engagement?" or "How to improve engagement for a segment?" - behavior-based segmentation
- **Persona (WHO):** User demographics, context, needs
- **Behavior (HOW):** Engagement patterns, content preferences, usage frequency
- **Value (WHY):** Engagement value (time spent, actions), retention value, strategic value
- **Rank & Focus:** Engagement impact × Segment size × Value → Focus on high-engagement segments

**Concrete Examples:**
- "Content engagement: Segment by content type (friends vs creators) × Behavior (engagement patterns) × Value (time spent, retention) → Rank → Focus on high-engagement content"
...
### BUCKET 4: Churn Analysis Segmentation
**Questions:** ~2 | **Priority:** 🟡 YELLOW

**Board Slide Bullets:**
- **What:** "Segment users for churn analysis" or "Identify churn patterns" - segmentation to understand churn
- **Persona (WHO):** User demographics, lifecycle stage, acquisition source
- **Behavior (HOW):** Usage patterns before churn, engagement decline, feature usage
- **Value (WHY):** Churn risk, revenue at risk, strategic importance
- **Rank & Focus:** Churn risk × Segment size × Value → Focus on high-risk segments

**Concrete Examples:**
- "Churn segmentation: Segment by Persona (demographics, lifecycle) × Behavior (usage patterns, engagement decline) × Value (churn risk, revenue at risk) → Rank → Focus on high-risk segments"
...
1. **Classic User/Customer Segmentation** → Study Bucket 1, practice 5 questions
2. **Market/Geographic Segmentation** → Study Bucket 2, practice 5 questions

### 🟡 YELLOW (High-yield but shaky - Practice questions)
3. **Engagement/Behavior Segmentation** → Study Bucket 3, practice 5 questions
4. **Churn Analysis Segmentation** → Study Bucket 4, practice 5 questions
...
- [ ] Walk through segmentation framework in 2 minutes (Persona × Behavior × Value → Rank → Focus)
- [ ] Explain the difference between P5 (define segments) and P1 (segment to find hot spot)
- [ ] Identify when to use segmentation (user focus, market selection, engagement analysis, churn)
- [ ] Rank segments by impact, size, and effort
- [ ] Focus on top segment while considering secondary segments
...
**You're ready when:**
- You can explain the P5 framework to a non-technical person in 2 minutes
...
- You focus on **Persona × Behavior × Value framework, ranking, and focusing on top segments**, not memorizing answers

**Remember:** P5 is about defining and ranking segments to focus on. The framework: (Persona × Behavior × Value) → Rank → Focus Top Segments.
